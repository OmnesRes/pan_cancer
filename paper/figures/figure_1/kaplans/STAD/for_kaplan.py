...
f=open(os.path.join(BASE_DIR,'figures','figure_1','clustergrams','STAD','column_order.txt'))
##double loop list comprehension
new_order=[int(j) for i in f for j in i.strip().split()]

##get the original order of the patients
f=open(os.path.join(BASE_DIR,'figures','figure_1','clustergrams','STAD','for_clustering_100_good_100_bad.txt'))
...
##split the groups at TCGA-BR-A4J8


finaldata=[]
for i in clinical_and_files:
    if i[0] in final_order[:final_order.index('TCGA-BR-A4J8')]:
        finaldata.append([i[1],i[2],'1'])
    elif i[0] in final_order[final_order.index('TCGA-BR-A4J8'):]:
