##A script for finding the average Cox coefficient of different gene programs for each cancer
...
for i in pathways:
    temp=[]
    for j in all_cancers:
        ##get the average Cox coefficient
        temp.append(sum([j[k] for k in j if k in i])/float(len([j[k] for k in j if k in i])))
    final_array.append(temp)



names=['BLCA','BRCA','CESC','COAD','GBM','HNSC','KIRC','KIRP','LAML','LGG','LIHC','LUAD','LUSC','OV','SKCM','STAD']
pathway_names=['Proliferation_DNA_Repair', 'Immunecell', 'Tumor_suppressing_miRNA_targets', 'MES_ECM', 'MYC_targets_TERT', 'Squamous_differentiation_development', 'Estrogen_signaling', 'FOXO_stemness', 'Cellcell_adhesion', 'Fatty_acid_oxidation', 'Interferon', 'Hypoxia_glycolysis', 'Neural_signaling', 'Plasma_membrane_cellcell_signaling', 'EGF_signaling', 'MAPKs', 'Basal_signaling', 'Vesicle', 'amplicon_1Q', 'TAL1', 'Antiapoptosis', 'amplicon_16Q22']


##don't want any trailing tabs
f=open('average_coefficients.txt','w')
f.write('\t')
for i in names[:-1]:
    f.write(i)
    f.write('\t')
f.write(names[-1])
f.write('\n')
for i,j in zip(pathway_names,final_array):
    f.write(i)
    f.write('\t')
    for k in j[:-1]:
        f.write(str(k))
